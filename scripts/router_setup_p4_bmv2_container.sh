...
#GARRETT PARZYCH
#modified from the fabric example
#it's missing a few steps, in particular the docker image does not have the most up-to-date version of p4c so it must be updated to compile my program
#I have provided the json of compiled code in this folder because it seems machines with ipv6 have trouble with updating, so in a worst case scenario just place 
#the pre-compiled code into the docker
...
#complite
docker exec -w /root/ fabric_p4 sh -c 'mkdir PROJECT'
docker exec -w /root/PROJECT/ fabric_p4 sh -c 'mkdir build'
sudo docker cp project.p4 fabric_p4:/root/PROJECT/project.p4
docker exec -w /root/PROJECT/ fabric_p4 sh -c 'p4c --p4runtime-files build/project.txt --target bmv2 --arch v1model -o build project.p4'
docker exec -d -it fabric_p4 sh -c 'simple_switch /root/PROJECT/build/project.json'
